...
xloadsparefont()
{
	...
	FcResult result;
	 ...
	 // add font 2 to font cache as first 4 entries 
	if( font2[0] == '-' )
		fontpattern = XftXlfdParse(font2, False, False);
	else 
		fontpattern = FcNameParse((FcChar8 *)font2);
	if (fontpattern) {
		// Allocate the memory for new cache entries 
		frccap += 4;
...
xmakeglyphfontspecs(XftGlyphFontSpec *specs, const Glyph *glyphs, int len, int x, int y)
{
    ...
    /* Harbuzz transformation for ligatures */ 
...
xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og, Line line, int len)
{
	...
	xdrawline(line, 0, oy, len); //Fix the broken ligatures
